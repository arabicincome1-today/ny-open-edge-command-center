...
Download real NQ candles:

```bash
python3 download_candles.py --symbol NQ=F --interval 1m --period 5d --out NQ_1m.csv
```

Run the dashboard on those candles:
...
1. Log setup features.
2. Label whether each setup hit target or stop first.
3. Train a model to score setup quality.
4. Use the model as a filter before alerts.
